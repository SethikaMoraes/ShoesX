...
const AuthManager = {
  ...
  openAuthModal(mode) {
    ...
    const authSwitchRow = document.getElementById('auth-switch-row');
  ...
  async signIn(email, password) {
    ...
    try {
      if (AdminManager.signIn(email, password, { redirect: true })) {
...
// Admin Manager (local credentials for admin panel access)
const AdminManager = {
  storageKey: 'shoesx_admin_session',
  ...
  loadSession() {
    try {
      ...
      console.warn('Unable to load admin session:', e);
    }
  },
  ...
  signIn(username, password, options = {}) {
    ...
    const isAdminUser = normalized === 'admin' || normalized === 'admin@gmail.com';
